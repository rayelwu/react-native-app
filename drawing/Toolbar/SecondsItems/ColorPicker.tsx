...
function ColorPicker({ style }: ColorPickerProps) {
  ...
  const translateX = useRef(100);
  const translateY = useRef(100);
  ...
  return visible ? (
    <View style={[style]}>
      ...
      >
        <Canvas style={styles.colorWheel}
          onTouchMove={({ nativeEvent }) => {

            const { theta, radius } = canvas2Polar(vec(nativeEvent.locationX, nativeEvent.locationY), center);
          ...
          <Group style="stroke" strokeWidth={5}>
            <Circle
              ...
              cx={translateX.current}
              cy={translateY.current}
            />
          </Group>
          <Circle
            r={(currentSize * 0.5) + 5}
